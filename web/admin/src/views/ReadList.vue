...
const columns = [
  ...
  { title: '操作', key: 'actions', width: 120, render: (row: any) => h(NButton, { size: 'small', type: 'error', onClick: () => deleteRead(row.id) }, () => '删除') }
]
...
<template>
  <div>
    ...
    <n-data-table :columns="columns" :data="reads" :loading="loading" remote :pagination="pagination" @update:page="pagination.page = $event; loadReads()" />
  </div>
</template>
